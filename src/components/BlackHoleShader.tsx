...
export interface BlackHoleShaderProps {
...
export const BlackHoleShader: React.FC<BlackHoleShaderProps> = ({
  rotationSpeed = 0.3,
  diskIntensity = 1.0,
  starsOnly = 0.0,
  tilt = 0.0,
...
// Lightweight 2-octave noise for subtle effects (nebula etc.)
...
vec3 starField(vec3 rd) {
    ...
    // Subtle nebula (lite noise — pow(3) crushes detail anyway)
...
vec3 bbColor(float t) {
    t = clamp(t, 0.0, 2.5);
    vec3 lo = vec3(1.0, 0.18, 0.0);    // deep red     (~1500K)
    vec3 mi = vec3(1.0, 0.55, 0.12);   // orange       (~3500K)
    vec3 hi = vec3(1.0, 0.93, 0.82);   // warm white   (~8000K)
    vec3 hot = vec3(0.65, 0.82, 1.0);  // cool blue-wh (~15000K)
...
void main() {
    ...
    const render = (now: number) => {
      ...
      const f = 0.02; // Smoothing factor
      
      currentProps.rotationSpeed += (target.rotationSpeed - currentProps.rotationSpeed) * f;
      currentProps.diskIntensity += (target.diskIntensity - currentProps.diskIntensity) * f;
      currentProps.tilt += (target.tilt - currentProps.tilt) * f;
      ...
      gl.uniform1f(uStarsOnly, starsOnly);
      gl.uniform1f(uTilt, tilt);
      gl.uniform1f(uRotate, rotate);
